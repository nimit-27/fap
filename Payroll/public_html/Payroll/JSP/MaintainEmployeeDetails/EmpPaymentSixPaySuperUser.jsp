...
<%! int i=0,j=0 ;%>
<% i = 0; j=0; %>
<html:form action="EmpSixPayActionSuper" name="frmEmpPayForm" type="FCIPAY.Payroll.WEBTIER.Form.EmpPaySixPayForm" scope="session">
...
<TABLE cellSpacing=0 cellPadding=0 border=0 >
	...
	<TR height=30>
		<TD colSpan=11 class=mainHeader height=10>DEFINE EMPLOYEE CAFETERIA APPROACH SUPERUSER</TD>
	</TR>
...
<html:hidden name="frmEmpPayForm" property="screenName" value="EmpSixPayScreenSuperUser" />
...
function lfnLoad()
{
  ...
  //alert("screenmode=="+screenmode+"=mode1=="+mode1)
   document.all.butInsertMode.disabled="false"; 
...
function SaveRecord()
{
	...
	if(mandatory_Check("frmEmpPayForm"))
	{
		...
		if(iTr2 > 1)
		{
	        ...
	        document.all.butSave.disabled="true";
		    document.all.butSave.src="../INCLUDES/IMAGES/save_dis.jpg";
			frmEmpPayForm.action = "../../../EmpSixPayActionSaveActionSuper.do";
...
function doBaseAddRow()
{
  if(mandatory_Check("frmEmpPayForm"))
  {
		...
		document.forms[0].action = "../../../EmpSixPayActionAddRowActionSuper.do";
			...
			if (eval(paycode_i) != eval(chld_allwn))
			{
				for(j=i+1; j < (iTr2-1); j++)
				{
					...
					if((document.getElementsByName("status")[j].value != 'D'))
					{
						...
						if( (eval(paycode_i) == eval(trns_allwn)) && (eval(paycode_j) == eval(cony_allwn)))
						{
							count++;
							rows =delrowcnt +" or "+ (delrowcnt+1);
						}
						if( (eval(paycode_i) == eval(cony_allwn)) && (eval(paycode_j) == eval(trns_allwn)))
						{
							count++;
							rows =delrowcnt +" or "+ (delrowcnt+1);
						}
...
function doQueryMode()
{
	...
	frmEmpPayForm.action="../../../EmpSixPayActionQueryModeSuper.do";
	frmEmpPayForm.submit();
}
...
function doInsertMode()
{
	...
	document.all.userBtn.value = "false";
	frmEmpPayForm.action="../../../EmpSixPayActionNewModeSuper.do"
	frmEmpPayForm.submit();
}
...
function getHeaderRecord()
{
	if(mandatory_Check("frmEmpPayForm"))
	{
		...
		frmEmpPayForm.action="../../../EmpSixPayActionGetHeaderActionSuper.do"
		frmEmpPayForm.submit();
	}
}
...
function GetDetail()
{
	if(mandatory_Check("frmEmpPayForm"))
	{
		  ...
		  document.all.screenMode.value='U'; 
		  document.all.userBtn.value = "true";
		  frmEmpPayForm.action="../../../EmpSixPayActionDetailActionSuper.do";
		  frmEmpPayForm.submit();
	}
}
...
{
	document.all.buttonFlag.value='';
	frmEmpPayForm.action="../../../EmpSixPayActionRefreshSuper.do"
	frmEmpPayForm.submit();
}
...
function doBaseDelRow()
{
	...
	*/  
	...
	document.all.buttonFlag.value='';
	clrAdjustPerc();
	document.forms[0].action = "../../../EmpSixPayActionDeleteRowActionSuper.do";
	document.forms[0].submit();
}
...
function JumpDetail()
{
    ...
    frmEmpPayForm.action = "../../../EmpSixPayActionNextDetailActionSuper.do";
    frmEmpPayForm.submit();
...
function doGetNextHeader()
{
	...
	frmEmpPayForm.action="../../../EmpSixPayActionGetNextHeaderSuper.do";
	frmEmpPayForm.submit();
}
