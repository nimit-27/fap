...
<TBODY>
  ...
  <table cellspacing=0 cellpadding=0 border=0>
      ...
      <TABLE cellspacing="0" cellPadding="0">
           <TR>
			  ...
			  <td bgcolor="#4682B4" nowrap class=actTabText>Increment Re-instate<img src="../INCLUDES/IMAGES/pixel.gif" width="10" height="1" border="0"></td>
 ...
 <!-- Detail table Container starts -->
		 ...
		 <logic:equal property="penaType" name="frmEmpPenalityForm" value="PENALTY$STOPINCR">
				 <logic:equal property="txtIncrReinstFlg" name="frmEmpPenalityForm" value="Y">
					<td class=labelText>Waiver From</td>
					<td colspan="1">
					<html:text  property="incrWaveFrm"  maxlength="10" size="23" styleClass="required" onchange="chkValidDateFrm(this)" />
					<logic:equal property= "screenMode" name= "frmEmpPenalityForm" value="U">  
					<img src="..\INCLUDES\IMAGES\cal.gif" title="Calender" width=15 height=19 align="top" styleClass="lov" name="effDate" alt="<%=(i-1) %>" onclick="showCalendar('incrWaveFrm',this)" />
					</logic:equal>
					</td>
					<td class=labelText>Waiver To</td>
					<td colspan="1">
					<html:text  property="incrWaveTo"  maxlength="10" size="23" styleClass="required" onchange="chkValidDateTo(this);" />
					<logic:equal property= "screenMode" name= "frmEmpPenalityForm" value="U">  
					<img src="..\INCLUDES\IMAGES\cal.gif" title="Calender" width=15 height=19 align="top" styleClass="lov" name="effDateTo" alt="<%=(i-1) %>" onclick="showCalendar('incrWaveTo',this)" />
					</logic:equal>
					</td>
				 </logic:equal>
				 <logic:notEqual property="txtIncrReinstFlg" name="frmEmpPenalityForm" value="Y">
					<td class=labelText>Waiver From </td>
					<td colspan="1" >
						<html:text  property="incrWaveFrm"  maxlength="10" size="23" styleClass="required" onchange="chkValidDateFrm(this)" readonly="true"/>
						<img src="..\INCLUDES\IMAGES\cal.gif" title="Calender" width=15 height=19 align="top" styleClass="lov" name="effDate" onclick="showCalendar('incrWaveFrm',this)" />
					</td>
					<td class=labelText>Waiver To </td>
					<td colspan="1">
						<html:text  property="incrWaveTo"  maxlength="10" size="23" styleClass="required" onchange="chkValidDateTo(this);" readonly="true"/>
						<img src="..\INCLUDES\IMAGES\cal.gif" title="Calender" width=15 height=19 align="top" styleClass="lov" name="effDateTo" alt="<%=(i-1) %>" onclick="showCalendar('incrWaveTo',this)" />
					</td>
				 </logic:notEqual>
		 </logic:equal>
...
<!-- Bottom Blue Bar -->
      <TR>
		 ...
		 <logic:equal property="penaType" name="frmEmpPenalityForm" value="PENALTY$STOPINCR">
			 <logic:equal property="screenMode" name="frmEmpPenalityForm" value="U">
				 <logic:equal property="txtIncrReinstFlg" name="frmEmpPenalityForm" value="Y">
					<html:button value='Increment Re-Instate' disabled="true" property="butInstIncr" styleClass="bottomBarText" />
				 </logic:equal>
				 <logic:notEqual property="txtIncrReinstFlg" name="frmEmpPenalityForm" value="Y">
					<html:button value='Increment Re-Instate' onclick="reInstIncrement()" property="butInstIncr" styleClass="bottomBarText" />
				 </logic:notEqual>
			 </logic:equal>
...
<html:text property="newPenaEffDate" name="frmEmpPenalityForm"  />
<html:text property="txtPenaEffDateTo" name="frmEmpPenalityForm"  />
...
function chkValidDateFrm()
{
	var penFrmAct = frmEmpPenalityForm.newPenaEffDate.value;
	var penToAct = frmEmpPenalityForm.txtPenaEffDateTo.value;
	var incrWaveFrm = frmEmpPenalityForm.incrWaveFrm.value;
	var incrWaveTo = frmEmpPenalityForm.incrWaveTo.value;
	//alert("incrWaveFrm="+incrWaveFrm+"incrWaveTo="+incrWaveTo)
	if (!cmpDate(penFrmAct,incrWaveFrm,0))
	{
		alert("Penalty imposed from date is, "+penFrmAct+". Wave off start date can not be entered before the penalty imposed date."); 
		frmEmpPenalityForm.incrWaveFrm.value="";
		return false;                      
	}
}
function chkValidDateTo()
{
	var penFrmAct = frmEmpPenalityForm.newPenaEffDate.value;
	var penToAct = frmEmpPenalityForm.txtPenaEffDateTo.value;
	var incrWaveFrm = frmEmpPenalityForm.incrWaveFrm.value;
	...
	if (incrWaveFrm=="") 
	{
		alert("Please enter wave from date first.");
		frmEmpPenalityForm.incrWaveTo.value="";
		return;
	}
	if (penToAct=="") 
	{

		/*if(incrWaveToYYMM >= curryr)
		{
			alert("This penalty can be waved till last month. Otherwise go to penalty detail tab and end date the running penalty.");
		}*/
		return;
	}
	if (!cmpDate(incrWaveTo,penToAct,0))
	{
		alert("Penalty imposed upto date is, "+penToAct+". Wave off upto date can not be greater than actual penalty end date."); 
		frmEmpPenalityForm.incrWaveTo.value="";
		return false;                      
	}
	if (!cmpDate(incrWaveFrm,incrWaveTo,0))
	{
		alert("Wave off start date can not be greater than Wave off end date."); 
		frmEmpPenalityForm.incrWaveFrm.value="";
		frmEmpPenalityForm.incrWaveTo.value="";
		return false;                      
	}
...
function lfnLoad()
{
    ...
    if(mode=='U')
    {
		...
		{
			if(penaType!='PENALTY$STOPINCR')
			{
				alert("Increment is not stopped for this employee. So re-instatement is not allowed to perform.");
			}
		}
    }
...
{
    document.all.buttonFlag.value='';
	document.getElementById("hdnAction").value = "GET_HDR_INCR";
		document.all.butExecute.disabled="true";
		document.all.butExecute.src="../INCLUDES/IMAGES/execute_dis.jpg";		

    frmEmpPenalityForm.action="../../../EmpPenalityIncrWaveHeaderAction.do"
    frmEmpPenalityForm.submit();
}
...
{ 
    ...
    frmEmpPenalityForm.action="../../../PenalityIncrWaveGetNextHeader.do";
    frmEmpPenalityForm.submit();
}
...
function PrevHeader()
{
    document.all.buttonFlag.value='';
    frmEmpPenalityForm.userPositionRequested.value = 1*frmEmpPenalityForm.positionRequested.value - 1;
    frmEmpPenalityForm.action = "../../../PenalityIncrWaveGetPrevHeader.do";
    frmEmpPenalityForm.submit();
}
...
function enableAll()
{
	...
	frmEmpPenalityForm.incrWaveFrm.disabled="";
}
...
function checkAdhocPromEffDate(cdate)
{
    ...
    if(frmEmpPenalityForm.incrWaveFrm.value!= '')
    {
        ...
        {
            var arr=cdate.split("-");
            var arrDOJF=frmEmpPenalityForm.incrWaveFrm.value.split("-");
            var cyear= arr[2];            
            var dyear=arrDOJF[2];
...
{   
	if(frmEmpPenalityForm.screenMode.value == 'N')
	{
		...
		{       
			...
			document.getElementById("incrWaveFrm").style.background="#ffffd3";
			document.getElementById("incrWaveFrm").className="required";
		...
		else if(frmEmpPenalityForm.penaType.value=='PENALTY$RECVRY')
		{
			...
			document.getElementById("incrWaveFrm").value = "";
			document.getElementById("incrWaveFrm").style.background="#e0e0e0";
			document.getElementById("incrWaveFrm").className="locked";
...
function reInstIncrement()
{
	if(confirm("Do you want to Re-instate the increment.?"))
	{
		if(mandatory_Check("frmEmpPenalityForm"))
		{
			document.getElementById("hdnAction").value = "REINST_INCR";
			document.forms[0].action="../../../IncrReinstateAction.do";
			document.forms[0].submit();
		}
	}
}  
