...
<jsp:include page="../../../COMMON/JSP/menu.jsp" flush="true" />
  ...
  <html:form  action="MonthlyPayroll" name="frmMonthlyPayroll" type="FCIPAY.Payroll.WEBTIER.Form.MonthlyPayrollForm" scope="session">
...
<table>
<TR height=30>
  <TD  colSpan=12 class=mainHeader height=10>COMPUTE MONTHLY PAYROLL PROCESS FOR <bean:write name="frmMonthlyPayroll" property="payYYMM" /></TD>
</TR>
</TABLE>
...
<!-- Bottom Blue Bar -->
        <TR>
               ...
               <td  bgcolor=#4682b4><html:button value=' Compute Monthly Payroll'  onclick="computeTransaction()" disabled="false" property="butInsert" styleClass="bottomBarText" /></td>
...
<!-- Container Table Ends -->
    ...
    <html:hidden name="frmMonthlyPayroll"  property="screenName" value="CompPayroll" />
    ...
    <html:hidden name="frmMonthlyPayroll"  property="loginLocCode" />
     <html:hidden name="frmMonthlyPayroll"  property="hdnMaxRebFlag" /> 
   ...
   if(!mandatory_Check("frmMonthlyPayroll"))
  {
    return false;
      ...
      if (frmMonthlyPayroll.hdnAction.value != 'SUB') 
      {
        ...
        ac="../../../ComputeMonthlyPayroll.do?param1="+document.all.txtEmp.value+"&param2=A";
...
function doQueryMode()
{
  ...
  document.forms[0].action="../../../MonthlyPayrollQueryMode.do";
  document.forms[0].submit();
}
...
function lfnLoad()
{
	...
	if (frmMonthlyPayroll.hdnAction.value=='S') 
	{
	  ...
	  if(mode != '')
  {
     var payYear = frmMonthlyPayroll.payYYMM.value;
...
{
  if (document.frmMonthlyPayroll.chkConsMaxReb.checked ==  true) 
  {
      document.frmMonthlyPayroll.hdnMaxRebFlag.value = 'Y';      
  }      
  else
      document.frmMonthlyPayroll.hdnMaxRebFlag.value = 'N';      
}
