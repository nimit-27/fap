...
<html:form  action="UndoDpsWrkDoneAction" name="frmDpsWrkDone" type="FCIPAY.Payroll.Labour.WEBTIER.Form.CalcDpsWrkDoneForm" scope="session">
...
<table>
  <TR height=30>
    ...
    <TD  colSpan=12 class=mainHeader height=10>UNDO LABOUR INECTIVE FOR <bean:write name="frmDpsWrkDone" property="payYYMM" /></TD>
    <%} else {  %>          
    
    <TD  colSpan=12 class=mainHeader height=10>UNDO LABOUR ACTUAL WORK DONE FOR <bean:write name="frmDpsWrkDone" property="payYYMM" /></TD>
    <%}  %>
...
<TBODY>
  <TR>
    <TD>
       ...
       <TR>
        <% if(empLbr.equals(new String("L"))) {%> 
          <TD class=subHeader colspan=6>Undo Labour Incentive</TD>
          <%} else {  %> 
          
          <TD class=subHeader colspan=6>Undo Labour Actual Workdone</TD>
          <%}  %>       
...
<!-- Bottom Blue Bar -->
        <TR>
               ...
               <td align=center bgcolor=#4682b4 width="30%"></td>
                <% if(empLbr.equals(new String("L"))) {%> 
                <td  bgcolor=#4682b4><html:button value='Undo Incentive'  onclick="if(checkFields()){UndoWrkDone()}else{return false;}" property="butCalcIncentive" styleClass="bottomBarText" /></td>
                <%} else {  %>          
                
                <td  bgcolor=#4682b4><html:button value='Undo Actual Work Done'  onclick="if(checkFields()){UndoWrkDone()}else{return false;}" property="butCalcIncentive" styleClass="bottomBarText" /></td>
                <%}  %>
...
<html:hidden name="frmDpsWrkDone"  property="screenName" value="UndoDpsWrkDoneScreen" />
...
function doQueryMode()
{
  ...
  frmDpsWrkDone.action="../../../UndoDpsWrkDoneQuery.do"
  frmDpsWrkDone.submit();
}
  


  function UndoWrkDone()
  {
      if (frmDpsWrkDone.hdnAction.value != 'SUB') 
      {
        frmDpsWrkDone.hdnAction.value = 'S'; 
        frmDpsWrkDone.action = "../../../UndoDpsWrkDone.do";
        frmDpsWrkDone.submit();
      }
      else
      {
