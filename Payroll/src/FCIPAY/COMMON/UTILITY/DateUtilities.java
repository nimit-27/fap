...
public int dateDiff2(String fromDate,String toDate) {
        Calendar cal=Calendar.getInstance();
        cal.set(Integer.parseInt(this.get(fromDate,this.YEAR)),Integer.parseInt(this.get(fromDate,this.MONTH))-1,Integer.parseInt(this.get(fromDate,this.DATE)));
        long fromDateInMilliSec =cal.getTime().getTime();
        cal.set(Integer.parseInt(this.get(toDate,this.YEAR)),Integer.parseInt(this.get(toDate,this.MONTH))-1,Integer.parseInt(this.get(toDate,this.DATE)));        
       ...
       /* if(fromDateInMilliSec==toDateInMilliSec) {
        ...
        int noOfDays=(int)((fromDateInMilliSec-toDateInMilliSec)/(24*3600*1000));
        return noOfDays;
    }    
    ...
    /**
     * returns array of String of all the dates from fromDate to toDate. toDate must be greater than fromDate
     * x = 1 ------ inclusive of toDate
     * x = 0 ------ exclusive of toDate
     **/
    public String[] getDates(int x, String fromDate, String toDate)
    {
        String[] datesArray = null;
        if(x == 0)
            datesArray = new String[this.dateGTcurrent(fromDate, toDate)];
        else if(x == 1)
            datesArray = new String[this.dateGTcurrent(fromDate, toDate) + 1];
        else
            return null;
        Calendar oCalendar = Calendar.getInstance();
        oCalendar.set(Integer.parseInt(fromDate.substring(fromDate.lastIndexOf("/") + 1)), Integer.parseInt(fromDate.substring(fromDate.indexOf("/") + 1, fromDate.lastIndexOf("/"))) - 1, Integer.parseInt(fromDate.substring(0, fromDate.indexOf("/"))));
        for(int i=0; i<datesArray.length; i++)
        {
            datesArray[i] = oCalendar.get(oCalendar.DATE) + "/" + (oCalendar.get(oCalendar.MONTH) + 1) + "/" + oCalendar.get(oCalendar.YEAR);
            oCalendar.set(Integer.parseInt(fromDate.substring(fromDate.lastIndexOf("/") + 1)), Integer.parseInt(fromDate.substring(fromDate.indexOf("/") + 1, fromDate.lastIndexOf("/"))) - 1, Integer.parseInt(fromDate.substring(0, fromDate.indexOf("/"))) + i + 1);
        }
        return datesArray;
    }

    /**
     * returns ordered array of String of dates
     **/
    public String[] orderDates(String[] tempDates)
    {
        ...
        for(int i=0; i<tempDates.length; i++)
        {
            for(int j=0; j<i; j++)
            {
                if(Integer.parseInt(tempDates[i].substring(tempDates[i].lastIndexOf("/")+1)) < Integer.parseInt(tempDates[j].substring(tempDates[j].lastIndexOf("/")+1)))
                {
                    date = tempDates[j];
                    tempDates[j] = tempDates[i];
                    tempDates[i] = date;
                }
                else if((Integer.parseInt(tempDates[i].substring(tempDates[i].lastIndexOf("/")+1)) == Integer.parseInt(tempDates[j].substring(tempDates[j].lastIndexOf("/")+1))) && (Integer.parseInt(tempDates[i].substring(tempDates[i].indexOf("/")+1, tempDates[i].lastIndexOf("/"))) < Integer.parseInt(tempDates[j].substring(tempDates[j].indexOf("/")+1, tempDates[j].lastIndexOf("/")))))
                {
                    date = tempDates[j];
                    tempDates[j] = tempDates[i];
                    tempDates[i] = date;
                        ...
                        && (Integer.parseInt(tempDates[i].substring(0, tempDates[i].indexOf("/"))) < Integer.parseInt(tempDates[j].substring(0, tempDates[j].indexOf("/")))))
                {
                    date = tempDates[j];
                    tempDates[j] = tempDates[i];
                    tempDates[i] = date;
    ...
    /**
     * returns the date that comes 'days' days after 'tempDate'
     **/
    public String getDate(String tempDate, int days)
    {
        String date = null;
        Calendar oCalendar = Calendar.getInstance();
        oCalendar.set(Integer.parseInt(tempDate.substring(tempDate.lastIndexOf("/") + 1)), Integer.parseInt(tempDate.substring(tempDate.indexOf("/") + 1, tempDate.lastIndexOf("/"))) - 1, Integer.parseInt(tempDate.substring(0, tempDate.indexOf("/"))) + days);
        date = oCalendar.get(oCalendar.DATE) + "/" + (oCalendar.get(oCalendar.MONTH) + 1) + "/" + oCalendar.get(oCalendar.YEAR);
        return date;
    }
